class TcpNetworkFilter:
    ...
    def _is_valid_tcp_response_dest(self, segment: bytes, server_ip: str) -> bool:
        if server_ip != self._resp_source_ip:
            return False
        source_resp_port: int = int.from_bytes(segment[:2], byteorder="big")
        dest_resp_port: int = int.from_bytes(segment[2:4], byteorder="big")
        return (
            dest_resp_port == self._resp_dest_port
            and source_resp_port == self._resp_source_port
        )
    ...
    def get_tcp_segment_from_ip_packet(packet: bytes) -> bytes:
        ...
        total_len = int.from_bytes(packet[2:4], byteorder="big")
        if total_len == 0:
            total_len = len(packet)

        end = min(len(packet), total_len)

        if ip_header_len > end:
            return b""

        return packet[ip_header_len:end]
