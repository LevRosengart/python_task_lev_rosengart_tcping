...
class TcpPinger:
    def __init__(self, server_ip: str, server_port: int):
        ...
        self._client_ip: str = self._ip_resolver.client_ip if server_ip != "127.0.0.1" else "127.0.0.1"
        self._client_socket: socket.socket = socket.socket(
            socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_TCP
        )
        self._client_socket.settimeout(3)
    ...
    def ping(self) -> float:
        ...
        self._client_socket.sendto(syn_segment.tcp_syn_segment, (self._server_ip, self._server_port))
        tcp_received_segment: bytes = b""
        while True:
            ...
            if self._net_filter.is_valid_tcp_response(tcp_received_segment, served_addr, syn_segment.seq_num):
                break
